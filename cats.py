...
def about(topic):
    ...
    def checker(paragraph):
        new = remove_punctuation(paragraph)
        new = lower(new)
        new = split(new)
...
def accuracy(typed, reference):
    ...
    for i in typed_words:
        if index > (len(reference_words) - 1):
            return (correct/len(typed_words)) *100 
        if i == reference_words[index]:
            correct += 1
            total += 1
        if i != reference_words[index]:
            total += 1
...
def feline_fixes(typed, reference, limit):
   """A diff function for autocorrect that determines how many letters
    ...
    """
    # BEGIN PROBLEM 6
   def helper (typed,reference,changes):
    typed_word = list(typed)
    reference_word = list(reference)
    if typed_word == [] or reference_word == [] or typed_word == reference_word:
     return 0 
    if changes > limit:
        return limit + 1
   
    first_typed_letter = typed_word[0]
    rest_typed_word = typed_word[1:]
   
    first_reference_letter = reference_word[0]
    rest_reference_word = reference_word[1:]
    
    if first_typed_letter == first_reference_letter:
     value = helper(rest_typed_word,rest_reference_word,changes) 

    if first_typed_letter != first_reference_letter:
     value = helper(rest_typed_word,rest_reference_word,changes+1) + 1
    return value 


   length = abs(len(typed) - len(reference))

   if helper(typed,reference, length) + length <= limit:
    return helper(typed,reference, length) + length
   else:
    return limit + 1 
...
def hidden_kittens(typed, reference, limit):
    ...
    def helper(typ, ref, lim):
        ...
        if ref[0] == typ[0]:
            rest = helper(typ[1:], ref[1:], lim)
            b = helper(typ[1:], ref, lim - rest)
            return rest+b
        else:
            return helper(typ[1:], ref, lim)

    if helper(typed, reference, limit ) == 0:
        return limit + 1
    else:
        return helper(typed, reference, limit)
...
def report_progress(typed, prompt, user_id, upload):
    ...
    ratio = 0
    if typed == [] or prompt == []:
        print ('ID: ' f"{user_id}" ' Progress: ' f"{ratio/total}")
        return print (f"{ratio/total}")
    if typed [0] != prompt [0]:
        print ('ID: ' f"{user_id}" ' Progress: ' f"{ratio/total}")
        return print (f"{ratio/total}")
    while typed [0] == prompt[0] :
        ratio = ratio + 1
        typed = typed [1:]
        prompt = prompt [1:]
...
def time_per_word(words, times_per_player):
    ...
    player_range = range(len(times_per_player))
    times_per_word = [[times_per_player[player][ time + 1 ] - times_per_player[player][time]  for time in word_range ] for player in player_range ]
...
def fastest_words(match):
    ...
    for word in word_indices:
       player_list.append(min(player_indices, key = lambda x:match["times"][x][word]))
    for player in player_indices:
        word_list = []
        for index in range(len(word_indices)):
            if player == player_list[index]:
                 x = get_word(match,index)
                 word_list.append(x)
