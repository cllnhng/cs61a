...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        while current.is_hive == False :
            if self.lower_bound <= moves <= self.upper_bound and current.bees != []:
...
class LongThrower(ThrowerAnt):
    ...
    implemented = True
    lower_bound = 5
    upper_bound = float('inf')   # Change to True to view in the GUI
    # END Problem 4
...
class FireAnt(Ant):
    ...
    def reduce_health(self, amount):
        ...
        current_list = self.place.bees[:]
        super().reduce_health(amount)

        if self.health <= 0:
            number = amount + self.damage
        else:
            number = amount

        for bees in current_list:
            bees.reduce_health(number)
...
# END Problem 12
    ...
    def action(self, gamestate):
        ...
        ants_behind = self.ants_behind()
        
        for ant in ants_behind:
            if ant.doubled == False:
                ant.double()

            if ant.is_container == True:
                if ant.ant_contained != None:
                    if ant.ant_contained.doubled == False:
    ...
    def ants_behind(self):
        ...
        while spot.exit != None:
            spaces_behind_queen.append(spot.exit)
            spot = spot.exit
        for space in spaces_behind_queen:
            if space.ant != None:
                ants_behind_queen.append(space.ant)      
        return ants_behind_queen
