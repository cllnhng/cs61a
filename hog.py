...
def roll_dice(num_rolls, dice=six_sided):
    ...
    rollpoints = 0
    sowsaddectector = False
    while num_rolls > 0:
        value = dice()
        num_rolls = num_rolls - 1
        if value == 1:   
            sowsaddectector = True
        else:
            rollpoints = rollpoints + value
    if sowsaddectector == True: 
        return 1
    else:
        return rollpoints
...
def take_turn(num_rolls, player_score, opponent_score, dice=six_sided, goal=GOAL_SCORE):
    ...
    points = 0
    if num_rolls == 0:
        oinkpoints = oink_points(player_score, opponent_score)
        points = points + oinkpoints
    else:
        rollpoints = roll_dice(num_rolls, dice)
...
def pigs_on_prime(player_score, opponent_score):
    ...
    points = 0  
    if is_prime(player_score) == False:
            return 0
    if is_prime(player_score) == True:
        player_score = player_score + 1
        points = points + 1
        while is_prime(player_score) == False:
            player_score = player_score + 1
            points = points + 1
    return points
...
def play(strategy0, strategy1, score0=0, score1=0, dice=six_sided,
    ...
    while score0 < goal and score1 < goal :
        if who == 0:
            ...
            score0 += take_turn(roll0, score0, score1, dice, goal)
            if is_prime(score0):
                score0 +=  pigs_on_prime(score0,score1)

        elif who == 1:
            roll1 = strategy1(score1,score0)
            score1 += take_turn(roll1,score1, score0, dice, goal)  
            if is_prime(score1):
...
#def new_func(strategy0, player_score, opponent_score):
 #   strategy0(player_score,opponent_score)
...
def pigs_on_prime_strategy(score, opponent_score, threshold=8, num_rolls=6):
    ...
    """
    # BEGIN PROBLEM 11
    score1 = score + oink_points(score,opponent_score)
    pigs = 0
    if oink_points(score,opponent_score)>= threshold:
        return 0
    if is_prime(score1) ==  True:
        pigs = pigs_on_prime(score,opponent_score) 
    else:
        pigs = threshold + 1
    if pigs <= threshold:
        return 0
